...
with the added complexity of having to update several components everytime a 
new tool or change was introduced — overengineering the instructions explicitly
...
- **Guardrails**: an earlier version of this project
  ...
  `DirectAnswerTool` every time.- **Automated tests** for each tool in isolation, 
  and for the tool-selection
...
- **UI Imporvements** can be done -- both in terms of the aestetics, and in terms 
of the way the reponses/traces are printed out -- to accommodate streaming and 
spacing the responses to complex prompts which lead to calling multiple agents, 
and show tool-calling interdependencies.
